...
const TransferRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
      ...
      await axios.post(`${API_URL}/api/get-daily-ledger`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Daily Ledger </p>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Daily Ledger</h4> 
