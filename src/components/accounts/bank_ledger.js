...
const TransferRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
    ...
    if(selectedBank == null){
        swal({title:'select a bank account',icon:'warning'});return false;
    }

      let payload = {
          bankId: selectedBank != null? selectedBank.bank_acc_id:null,
          fromDate: byDateTime.fromDate,
          toDate: byDateTime.toDate
      }

      reportLoadingSet(true)
      await axios.post(`${API_URL}/api/get-bank-ledger`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Bank Account Ledger </p>

<p style={{float:'left',width:'30%',textAlign:'left',margin:0,padding:0,fontSize:'16px'}}>
{selectedBank != null ?`Bank Account : ${selectedBank!=null?selectedBank.bank_display_name:''}`:''}
</p>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Bank Account Ledger</h4> 
