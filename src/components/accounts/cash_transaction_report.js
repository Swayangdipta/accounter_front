...
const CashTransactionHistory = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
      ...
      let payload = {
          accountId: selectedAccount != null? selectedAccount.tran_acc_id:null,
      ...
      await axios.post(`${API_URL}/api/get-cash-transactions`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        reportSet(res.data)
        printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Others Transaction Report</p>
  ...
  <p style={{float:'left',width:'30%',textAlign:'left',margin:0,padding:0,fontSize:'16px'}}>
    ...
    {`Transaction Head : ${selectedAccount!=null?selectedAccount.tran_acc_name:''}`}
    </p>
        ...
        <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
          <TableHead>
            <TableRow> 
              ...
              <TableCell align="left" style={{width:'15%'}}>Account Head </TableCell>
          ...
          <TableBody>
              ...
              
              {
                report.map((row,sl)=>(
                  ...
                  <TableCell  align="left">{moment(row.cash_tran_created_isodt).format('Y MMM DD')}</TableCell>
                
                  <TableCell  align="left">{row.cash_tran_code}</TableCell>
                  <TableCell  align="left">{row.tran_acc_name}</TableCell>
                  <TableCell  align="left">{row.cash_tran_type}</TableCell>
                  <TableCell  align="left">{row.cash_tran_note}</TableCell>
                  <TableCell  align="left">{format(parseFloat(row.cash_tran_in_amount).toFixed(2))}</TableCell>
                  <TableCell  align="right">{format(parseFloat(row.cash_tran_out_amount).toFixed(2))}</TableCell>
                   </TableRow>
                ))
            ...
            <TableCell  align="left">{format(_.sumBy(report,curr => Number(curr.cash_tran_in_amount)).toFixed(2))}</TableCell>
            <TableCell  align="right">{format(_.sumBy(report,curr => Number(curr.cash_tran_out_amount)).toFixed(2))}</TableCell>
            </TableRow>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Others Transaction Report</h4> 
