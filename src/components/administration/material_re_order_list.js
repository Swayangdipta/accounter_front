...
const CustomerTransactionHistory = ({location,currentRoute,currentRouteSet,authInfo})=>{
 ...
 let getProducts = async()=>{
    ...
    await axios.post(`${API_URL}/api/get-material-current-stock-detail`,{'fetch-type':'low'},{headers:{'auth-token':authInfo.token}}).then(res=>{
      reportSet(res.data)
      printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Material Re-Order List </p>
        ...
        <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
          <TableHead>
            <TableRow> 
              ...
              <TableCell align="left" style={{width:'15%'}}>Material Code </TableCell>
              <TableCell align="left" style={{width:'15%'}}>Material Name </TableCell>
              <TableCell align="left" style={{width:'15%'}}>Material Category </TableCell>
          ...
          <TableBody>
              ...
              
              {
                report.map((row,sl)=>(
                  ...
                  <TableCell  align="right">{parseFloat(row.material_purchase_rate).toFixed(2)}</TableCell>
                  <TableCell  align="right">{parseFloat(row.material_re_order_lebel).toFixed(2)}</TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Material Re-Order  List</h4> 
