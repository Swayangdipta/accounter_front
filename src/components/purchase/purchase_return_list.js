...
const CustomerTransactionHistory = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
      ...
      await axios.post(`${API_URL}/api/get-purchase-return-list`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        reportSet(res.data)
        printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Purchase Return Record </p>

<p style={{float:'left',width:'30%',textAlign:'left',margin:0,padding:0,fontSize:'16px'}}>
{selectedSupplier != null ?`Supplier : ${selectedSupplier!=null?selectedSupplier.supplier_name:''}`:''}
<br/>
          ...
          <TableBody>
              ...
              
              {
                report.map((row,sl)=>(
                  ...
                  <TableCell  align="left">{moment(row.pur_return_created_isodt).format(dateTimeFormat)}</TableCell>
                
                  <TableCell  align="left">{row.pur_invoice_no}</TableCell>
                  <TableCell  align="left">{row.supplier_name} - {row.supplier_code}</TableCell>
                  <TableCell  align="left">{row.pur_return_note}</TableCell>

                  <TableCell  align="left">{row.prod_name}</TableCell>
                  <TableCell  align="left"> {format(parseFloat(row.pur_return_rate).toFixed(2))}</TableCell>
                  <TableCell  align="left">{row.pur_return_qty}</TableCell>
                  <TableCell  align="right">{format(parseFloat(row.pur_return_amount).toFixed(2))}</TableCell>
                   </TableRow>
                ))
            ...
            <TableCell  align="right">{format(_.sumBy(report,curr => Number(curr.pur_return_amount)).toFixed(2))}</TableCell>
            </TableRow>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Purchase Return  Record</h4> 
