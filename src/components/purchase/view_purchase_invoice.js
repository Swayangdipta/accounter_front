...
import PurchaseInvoice from './components/purchase_invoice';
...
const PurchaseInvoiceQuick = ({location,currentRouteSet,authInfo})=>{
    ...
    let getPurchaseInvoices = async ()=>{
        await axios.get(`${API_URL}/api/purchase-invoices`,{headers:{'auth-token':authInfo.token}}).then(res=>{
        ...
        <Autocomplete 
           ...
           onChange={(e,invoice)=>{
            selectedInvoiceSet(invoice)
              if(invoice != null){
                  purchaseIdSet(invoice.pur_id)
              }
           }}
           renderInput={(params) => <TextField 
   
       {...params} 
       label="Choose Purchase Invoice" 
       variant="outlined"
