...
const CustomerTransactionHistory = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
      ...
      await axios.post(`${API_URL}/api/customer-transaction-history`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        reportSet(res.data)
        printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Customer Transaction Report</p>

  <p style={{float:'left',width:'30%',textAlign:'left',margin:0,padding:0,fontSize:'16px'}}>
    Transaction Type : {`${selectedTranType.value}`} <br/>
    {selectedCustomer!=null?`Customer : ${selectedCustomer.customer_name}`:''}<br/>
    {selectedCustomer!=null?`Institution : ${selectedCustomer.customer_institution_name}`:''}
    </p>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Customer Transaction Report</h4> 
