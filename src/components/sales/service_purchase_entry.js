...
const PurchaseEntry = ({location,currentRoute,currentRouteSet,authInfo})=>{
    ...
    useEffect( ()=>{
      ...
      if(pathSpliter(location.pathname,3) != undefined){
        formActionSet('update')
         axios.post(`${API_URL}/api/get-expense-with-details`,{reqPayload:{purchaseId:parseFloat(pathSpliter(location.pathname,3))}},{headers:{'auth-token':authInfo.token}}).then(res=>{
            let purchase = res.data.message[0];
            let purchaseDetails = res.data.message[0].details;

            selectedEmployeeSet({
              employee_id: purchase.pur_ser_emp_id,
              employee_name: purchase.employee_name
            });

            selectedSupplierSet({
              supplier_id: purchase.pur_ser_supplier_id,
              supplier_name: purchase.pur_ser_supplier_type =='general'?'General Supplier':purchase.supplier_name,
              supplier_type: purchase.pur_ser_supplier_type,
              display_text: purchase.supplier_name
            })



            setPurchaseData({...purchaseData,invoice_no:purchase.pur_ser_invoice_no,
                supplier_name:purchase.supplier_name,display_text:purchase.supplier_name,
              supplier_mobile_no:purchase.supplier_mobile_no,supplier_address:purchase.supplier_address,
              pur_transport_cost:purchase.pur_ser_transport_cost,
              pur_supplier_type : purchase.pur_ser_supplier_type,
              pur_supplier_id : purchase.pur_ser_supplier_id
              
              })
              handleDateChangeSet(purchase.pur_ser_created_isodt)
              pur_vat_amount_set(purchase.pur_ser_vat_amount)
              pur_vat_percent_set(purchase.pur_ser_vat_percent)
              pur_discount_amount_set(purchase.pur_ser_discount_amount)
              pur_discount_percent_set(purchase.pur_ser_discount_percent)
              purTotalAmoSet(purchase.pur_ser_total_amount)
              purPaidAmoSet(purchase.pur_ser_paid_amount)
              purDueAmoSet(purchase.pur_ser_due_amount)



              selectedSupplierPayMethodSet({pay_method_id:purchase.pur_ser_pay_method,pay_method:purchase.pur_ser_pay_method})
              selectedSupplierPayAccSet({bank_display_name:purchase.bank_display_name,bank_acc_id:purchase.pur_ser_bank_id})

           let purchaseCartGet =  purchaseDetails.map((ele)=>{ 
              let product = {
                prod_id:ele.pur_ser_prod_id,
                prod_name:ele.prod_name,
                prod_purchase_rate:ele.pur_ser_rate,
                prod_sale_rate:ele.pur_ser_sale_rate,
                prod_qty:ele.pur_ser_qty,
                prod_total:ele.pur_ser_total_amount
              }
              return product;
            });
    ...
    let savePurchase = async ()=>{
       ...
       let purchase =  {...purchaseData,pur_vat_amount:pur_vat_amount,pur_vat_percent:pur_vat_percent,
        ...
        await axios.post(`${API_URL}/api${url}`,{purchase,purchaseCart},{headers:{'auth-token':authInfo.token}}).then(res=>{
          if(!res.data.error){
            let smg = 'Service Expense ';
            if(pathSpliter(location.pathname,3) != undefined){
              smg = 'Service Expense   updated '
            }
            swal({
              ...
              if(confirm){
                history.push(`/service/service-item-invoice/${res.data.message.purchaseId}`) 
                saveActionSet(false)
              }else{
    ...
    const getPurchaseInvoice = async ()=>{ 
          await axios.get(`${API_URL}/api/get-expense-invoce`,{headers:{'auth-token':authInfo.token}}).then(res=>{
    ...
    const getIndividualProducts = async()=>{
          await axios.post(`${API_URL}/api/get-individual-products`,{'select-type':'active',service:true},{headers:{'auth-token':authInfo.token}}).then(res=>{
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '2px'}}>Services Expense  Entry</h4> 
...
<Grid container spacing={3} >
        <Grid item xs={12} sm={8} >
          ...
          <h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '3px'}}>Supplier & Services  item information</h4>
          <Grid container spacing={3} >
                <Grid item xs={12} sm={6} className={classes.plusLinkDiv}>
                <Link className={classes.plusLink} to="/service/suppliers-manage" >+</Link>                 <Autocomplete 
                autoHighlight
                size="small"
              openOnFocus={true}
        ...
        <br/>
                <Grid container >
                      ...
                      <TextField style={{marginRight:'5px'}}  
                            ...
                            }}  type="number" label="expense  rate"  size="small" value={product.prod_purchase_rate} onChange={productHandle} name="prod_purchase_rate" variant="outlined" className={classes.inputField} />
                      </Grid>
                      <Grid item sm={1} >
                      </Grid>
          ...
          <Grid container>
            ...
            <Grid item xs={12}>
                  ...
                  <h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '3px'}}>Expense Item  Cart</h4>
                  <TableContainer >
      <Table className={classes.table} size="small" aria-label="a dense table">
        <TableHead>
          <TableRow>
            <TableCell align="left">SL</TableCell>
            <TableCell align="left">Expense Item</TableCell>
            <TableCell align="center">Expense  Rate</TableCell>
