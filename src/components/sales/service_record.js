...
const ServiceRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
    ...
    let [serviceProductDetail,serviceProductDetailSet] = useState([]);
    let [serviceProdCatDetail,serviceProdCatDetailSet] = useState([]);
    ...
    let serviceDelete = async (serviceId)=>{
        swal({
          ...
          if(confirm){
            await axios.post(`${API_URL}/api/service-delete`,{serviceId},{headers:{'auth-token':authInfo.token}}).then(res=>{
              if(!res.data.error){
               swal({
    ...
    let getSearchResult = async ()=>{
        ...
        await axios.post(`${url}`,{reqPayload},{headers:{'auth-token':authInfo.token}}).then(res=>{
            ...
            if(selectedSearchType != null && selectedSearchType.searchType=='By Product'){
              serviceProductDetailSet(res.data.message);
            ...
            if(selectedSearchType != null && selectedSearchType.searchType=='By Category'){
              ...
              records = _.chain(records)
                  ...
                  .map(service => {
                    return {
                      category_name: service[0].prod_cat_name,
                      products: _.chain(service)
                        .groupBy('service_d_prod_id')
                        .map(product => {
                          return {
                            prod_name: product[0].prod_name,
                            prod_code: product[0].prod_code,
                            quantity: _.sumBy(product, item => Number(item.service_qty))
                          }
                        })
                        .value()
                    }
                  ...
                  .value();
              serviceProdCatDetailSet(records)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Service Record </p>
...
<p style={{float:'right',width:'40%',textAlign:'right',margin:0,padding:0}}> Statement From { moment(byDateTime.dateTimeFrom).format(dateTimeFormat)  } <br/> TO { moment(byDateTime.dateTimeTo).format(dateTimeFormat) }
        ...
        {
  (selectedSearchType != null &&  selectedSearchType.searchType ==  'By Product' && serviceProductDetail.length>0 )?(
    ...
    <TableContainer >
      <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
        ...
        <TableBody>
            {
              serviceProductDetail.map((ele,index)=>(
                <TableRow  key={index}> 
                <TableCell  align="left">{index+parseFloat(1)}</TableCell>
                <TableCell align="left">{ele.service_invoice}</TableCell> 
                <TableCell align="left">{moment(ele.service_created_isodt).format(dateTimeFormat)}</TableCell>
                <TableCell align="left">{ele.customer_name}</TableCell>
                <TableCell align="right">{(ele.prod_name)}</TableCell>
                <TableCell align="right">{format((ele.service_rate).toFixed(2))}</TableCell>
                <TableCell align="right">{(ele.service_qty).toFixed(2)}</TableCell>
                 <TableCell align="right" className="print-no"> 
                 <Link to={{pathname:`/service/service-invoice/${ele.service_id}`}}> <ReceiptIcon style={{cursor:'pointer',color:'black',marginLeft:'2px'}} > </ReceiptIcon></Link>
               {
                 authInfo.role !='user'?(
                   <>
                <Link to={{pathname:`/service/service-update/${ele.service_id}`}}> 
        ...
        <TableRow colSpan={4}>
          ...
          <TableCell align="right"> { serviceProductDetail.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_qty);
...

{
  (selectedRecordType !=null && selectedSearchType != null &&  selectedSearchType.searchType !=  'By Product' && selectedSearchType.searchType !=  'By Category' && searchResult.length>0 &&  selectedRecordType.recordType=='Without Details')?(
    ...
    {/* service Record  without details */} 
    <TableContainer >
      <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
        <TableHead>
          <TableRow> 
        ...
        <TableBody>
            {
              searchResult.map((ele,index)=>(
                ...
                <TableCell  align="left">{index+parseFloat(1)}</TableCell>
                <TableCell align="left">{ele.service_invoice}</TableCell> 
                <TableCell align="left">{moment(ele.service_created_isodt).format('DD MMM Y')}</TableCell>
                <TableCell align="left">{ele.customer_name}</TableCell> 
                <TableCell align="left">{ele.employee_name}</TableCell>
                <TableCell align="left">{ele.user_full_name}</TableCell>
                <TableCell align="right">{ format(parseFloat(ele.service_subtotal_amount).toFixed(2))}</TableCell>
                <TableCell align="right">{format(parseFloat(ele.service_vat_amount).toFixed(2))} </TableCell>
                <TableCell align="right">{format(parseFloat(ele.service_discount_amount).toFixed(2))}</TableCell>
                <TableCell align="right">{format(parseFloat(ele.service_total_amount).toFixed(2))}</TableCell>
                <TableCell align="right">{format(parseFloat(ele.service_paid_amount).toFixed(2))}</TableCell>
                <TableCell align="right">{format(parseFloat(ele.service_due_amount).toFixed(2))}</TableCell>
                 <TableCell align="right" className="print-no">
  
                
                 <Link to={{pathname:`/service/service-invoice/${ele.service_id}`}}> 
...

                {
                  authInfo.role !='user'?(
                    <>
                     <Link to={{pathname:`/service/service-update/${ele.service_id}`}}>  
                     <EditIcon style={{cursor:'pointer',color:'green',marginLeft:'2px'}}  > </EditIcon></Link>
                <DeleteIcon onClick={()=>{serviceDelete(ele.service_id)}} style={{cursor:'pointer',color:'#FF0202',marginLeft:'2px'}} > </DeleteIcon>
                    </>
                  ):''
                }
        ...
        <TableRow colSpan={4}>
          ...
          <TableCell align="right">{ format( searchResult.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_subtotal_amount);
          },0).toFixed(2)) }</TableCell>
          <TableCell align="right">{ format( searchResult.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_vat_amount);
          },0).toFixed(2)) }</TableCell>
          <TableCell align="right">{ format( searchResult.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_discount_amount);
          },0).toFixed(2)) }</TableCell>
     
          <TableCell align="right">{ format(searchResult.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_total_amount);
          },0).toFixed(2)) }</TableCell>
          <TableCell align="right">{ format(searchResult.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_paid_amount);
          },0).toFixed(2)) }</TableCell>
          <TableCell align="right">{ format(searchResult.reduce((prev,curr)=>{
                return prev+parseFloat(curr.service_due_amount);
          },0).toFixed(2)) }</TableCell>
          <TableCell></TableCell>
        </TableRow>
      ...
      {
        ( selectedSearchType != null && selectedSearchType.searchType ==  'By Category' && serviceProdCatDetail.length>0)?(
          <Paper className={classes.paper} style={{marginTop:'1px',marginBottom:'1px'}} >
    ...
    <TableContainer >
      <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
        ...
        <TableBody>
            {
              serviceProdCatDetail.map((ele,index)=>(
                <>
        ...
        <TableBody>
            {
              searchResultDetails.map((ele,index)=>(
                ...
                <TableCell align="right">{format(ele.details[0].service_rate)}</TableCell>
                <TableCell align="right">{ele.details[0].service_qty}</TableCell>
                <TableCell align="right">{format((ele.details[0].service_prod_total).toFixed(2))}</TableCell>
             ...
             <EditIcon style={{cursor:'pointer',color:'green',marginLeft:'2px'}} > </EditIcon>
                  ...
                  
                {
                  ele.details.slice(1).map((detail,dIndex)=>(
                    ...
                    <TableCell  align="right">{format(detail.service_rate)}</TableCell>
                    <TableCell  align="right">{detail.service_qty}</TableCell>
                    <TableCell  align="right">{format((detail.service_prod_total).toFixed(2))}</TableCell>
                    </TableRow>
                  ))
                  ...
                  <TableCell  align="right">Total Quantity : {ele.details.reduce((prev,curr)=>{
                    ...
                    <TableCell  align="right">
                      Total : {format((ele.service_total_amount).toFixed(2))}<br/>
                   Paid :  {format((ele.service_paid_amount).toFixed(2))}<br/>
                   Due :  {format((ele.service_due_amount).toFixed(2))}
                  </TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '2px'}}>Service Record</h4>
...
export default connect(mapStateToPops,{currentRouteSet})(ServiceRecord); 