...
const PurchaseRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
    ...
    let purchaseDelete = async (purchaseId)=>{
        swal({
          ...
          if(confirm){
            await axios.post(`${API_URL}/api/service-expense-delete`,{purchaseId},{headers:{'auth-token':authInfo.token}}).then(res=>{
              if(!res.data.error){
               swal({
    ...
    let getSearchResult = async ()=>{
        ...
        let url = '';
            ...
            if(selectedRecordType != null && selectedRecordType.recordType == 'Without Details' ){
                url = `${API_URL}/api/get-service-item-purchase`
            }
                  ...
                  .map(sale => {
                    return {
                      ...
                      products: _.chain(sale)
                        .groupBy('pur_ser_d_prod_id')
                        .map(product => {
                          return {
                            prod_name: product[0].prod_name,
                            prod_code: product[0].prod_code,
                            quantity: _.sumBy(product, item => Number(item.pur_ser_d_qty))
                          }
                        })
                        .value()
                    }
    ...
    <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
      ...
      <TableBody>
          {
            purchaseProductDetail.map((ele,index)=>(
              ...
              <TableCell align="left">{ele.pur_ser_invoice_no}</TableCell> 
              <TableCell align="left">{moment(ele.pur_ser_created_isodt).format(dateTimeFormat)}</TableCell>
              <TableCell align="left">{ele.supplier_name}</TableCell>
              <TableCell align="right">{(ele.prod_name)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_rate).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_qty).toFixed(2)}</TableCell>
      ...
      <TableRow colSpan={4}>
        ...
        <TableCell align="right"> { purchaseProductDetail.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_qty);
...
(selectedRecordType !=null && selectedSearchType != null &&  selectedSearchType.searchType !=  'By Product' && selectedSearchType.searchType !=  'By Category' && searchResult.length>0 &&  selectedRecordType.recordType=='Without Details')?(
      ...
      <TableBody>
          {
            searchResult.map((ele,index)=>(
              ...
              <TableCell align="left">{ele.pur_ser_pay_method=='bank' ? ele.bank_display_name:'Cash'}</TableCell>
              <TableCell align="right">{(ele.pur_ser_subtotal_amount).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_vat_amount).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_discount_amount).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_transport_cost).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_total_amount).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_paid_amount).toFixed(2)}</TableCell>
              <TableCell align="right">{(ele.pur_ser_due_amount).toFixed(2)}</TableCell>
               <TableCell align="right" className="print-no">

              
               <Link to={{pathname:`/service/service-item-invoice/${ele.pur_ser_id}`}}> <ReceiptIcon style={{cursor:'pointer',color:'black',marginLeft:'2px'}} > </ReceiptIcon></Link>
               {
    authInfo.role !='user'?(
    <>
         <Link to={{pathname:`/service/service-purchase-update/${ele.pur_ser_id}`}}>  <EditIcon style={{cursor:'pointer',color:'green',marginLeft:'2px'}}  > </EditIcon></Link>
              <DeleteIcon onClick={()=>{purchaseDelete(ele.pur_ser_id)}} style={{cursor:'pointer',color:'#FF0202',marginLeft:'2px'}} > </DeleteIcon>        
   </>):''
 }
      ...
      <TableRow >
        ...
        <TableCell align="right">{ searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_subtotal_amount);
        },0).toFixed(2) }</TableCell>
        <TableCell align="right">{ searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_vat_amount);
        },0).toFixed(2) }</TableCell>
        <TableCell align="right">{ searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_discount_amount);
        },0).toFixed(2) }</TableCell>
        <TableCell align="right"> { searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_transport_cost);
        },0).toFixed(2) }</TableCell>
        <TableCell align="right">{ searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_total_amount);
        },0).toFixed(2) }</TableCell>
        <TableCell align="right">{ searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_paid_amount);
        },0).toFixed(2) }</TableCell>
        <TableCell align="right" >{ searchResult.reduce((prev,curr)=>{
              return prev+parseFloat(curr.pur_ser_due_amount);
        },0).toFixed(2) }</TableCell>
        <TableCell></TableCell>
      </TableRow>
  ...
  <TableContainer >
      ...
      <TableBody>
          {
            searchResultDetails.map((ele,index)=>(
              ...
              <TableCell align="right">{ele.details[0].pur_ser_rate}</TableCell>
              <TableCell align="right">{ele.details[0].pur_ser_qty}</TableCell>
              <TableCell align="right">{(ele.details[0].pur_ser_total_amount).toFixed(2)}</TableCell>
    ...
    authInfo.role !='user'?(
    <>
             <Link to={{pathname:`/service/service-purchase-update/${ele.pur_ser_id}`}}> 
           <EditIcon style={{cursor:'pointer',color:'green',marginLeft:'2px'}} > </EditIcon>
                ...
                
              {
                ele.details.slice(1).map((detail,dIndex)=>(
                  ...
                  <TableCell  align="right">{detail.pur_ser_rate}</TableCell>
                  <TableCell  align="right">{detail.pur_ser_qty}</TableCell>
                  <TableCell  align="right">{(detail.pur_ser_total_amount).toFixed(2)}</TableCell>
                  </TableRow>
                ))
                ...
                <TableCell  align="right">Total Quantity : {ele.details.reduce((prev,curr)=>{
                  ...
                  <TableCell  align="right">
                    Total : {(ele.pur_ser_total_amount).toFixed(2)}<br/>
                 Paid :  {(ele.pur_ser_paid_amount).toFixed(2)}<br/>
                 Due :  {(ele.pur_ser_due_amount).toFixed(2)}
                </TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '2px'}}>Service Expense  Record</h4>
