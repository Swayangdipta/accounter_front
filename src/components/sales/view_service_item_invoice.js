...
const PurchaseInvoiceQuick = ({location,currentRouteSet,authInfo})=>{
    ...
    let getPurchaseInvoices = async ()=>{
        await axios.get(`${API_URL}/api/service-item-invoices`,{headers:{'auth-token':authInfo.token}}).then(res=>{
  ...
  <Grid item xs={12} sm={12}>
        ...
        <Autocomplete 
           ...
           size="small"
           getOptionLabel={(option) => option.pur_ser_invoice_no}
           onChange={(e,invoice)=>{
            selectedInvoiceSet(invoice)
              if(invoice != null){
                  purchaseIdSet(invoice.pur_ser_id)
              }
           }}
           renderInput={(params) => <TextField 
   
       {...params} 
       label="Choose Service Item Invoice" 
       variant="outlined"
