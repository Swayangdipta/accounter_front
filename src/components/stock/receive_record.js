...
const ReceiveRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
      ...
      await axios.post(`${API_URL}/api/get-transfer-receives`,{payLoad:{transfer_b_from:obj.branchId,
        transfer_w_from:obj.warehouseId,from_date:obj.dateTimeFrom,to_date:obj.dateTimeTo
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Transfer Receive  Record</h4>
          ...
          <TableBody>
              
              {
                 ...
                 <TableCell align="left">{transfer.branch_name}</TableCell> 
                  ...
                  <TableCell align="right">{parseFloat(transfer.transfer_amount).toFixed(2)}</TableCell>
...
export default connect(mapStateToPops,{currentRouteSet})(ReceiveRecord); 