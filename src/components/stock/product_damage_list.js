...
const DamageList = ({location,currentRoute,currentRouteSet,authInfo})=>{
 ...
 let getDamages = async()=>{
  ...
  let getSearchResult = async ()=>{
      ...
      await axios.post(`${API_URL}/api/get-damages`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        reportSet(res.data)
        printSet(true)
        ...
        <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
          <TableHead>
            <TableRow> 
              ...
              <TableCell align="left" style={{width:'10%'}}>Damage Code</TableCell>
              <TableCell align="left" style={{width:'15%'}}>Product Code</TableCell>
              <TableCell align="left" style={{width:'15%'}}>Product Name</TableCell>
              <TableCell align="left" style={{width:'15%'}}>Damage Rate</TableCell>
              <TableCell align="right" style={{width:'15%'}}>Damage Quantity </TableCell>
              <TableCell align="right" style={{width:'15%'}}>Damage Total </TableCell>
          ...
          <TableBody>
              ...
              
              {
                report.map((row,sl)=>(
                  ...
                  <TableCell  align="left">{moment(row.customer_created_isodt).format(dateTimeFormat)}</TableCell>
                
                  <TableCell  align="left">  {row.damage_code}</TableCell>
                  <TableCell  align="left">  {row.prod_code}</TableCell>
                  <TableCell  align="left">{row.prod_name}</TableCell>
                  <TableCell  align="left">{ format(parseFloat(row.damage_rate).toFixed(2))}</TableCell>
                  <TableCell  align="right">{format(parseFloat(row.damage_qty).toFixed(2))}</TableCell>
                  <TableCell  align="right">{format(parseFloat(row.damage_total).toFixed(2))}</TableCell>
...
<TableRow  key=""> 
                  ...
                  <TableCell  align="right">{format(_.sumBy(report,(curr)=> Number(curr.damage_qty)).toFixed(2))}</TableCell>
                  <TableCell  align="right">{format(_.sumBy(report,(curr)=> Number(curr.damage_total)).toFixed(2))}</TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Damage  List</h4> 
...
export default connect(mapStateToPops,{currentRouteSet})(DamageList);