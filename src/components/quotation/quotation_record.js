...
const SalesRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
    ...
    const getCustomers = async ()=>{
          await axios.post(`${API_URL}/api/get-customers`,{'select-type':'active'},{headers:{'auth-token':authInfo.token}}).then(res=>{
            res.data.message.unshift({customer_id:0,customer_address:'',customer_mobile_no:'',customer_name:'General Customer'});
    ...
    let saleDelete = async (saleId)=>{
        swal({
          ...
          if(confirm){
            await axios.post(`${API_URL}/api/quotation-delete`,{saleId},{headers:{'auth-token':authInfo.token}}).then(res=>{
              if(!res.data.error){
               swal({
    ...
    let getSearchResult = async ()=>{
        ...
        let url = '';
            ...
            if(selectedRecordType != null && selectedRecordType.recordType == 'Without Details' ){
                url = `${API_URL}/api/get-quotations`
            }
            if(selectedRecordType != null && selectedRecordType.recordType == 'With Details' ){ 
                url = `${API_URL}/api/get-quotation-with-details`
            }
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Quotation Record </p>
  ...
  <p style={{float:'left',width:'30%',textAlign:'left',margin:0,padding:0,fontSize:'16px'}}>
    ...
    {selectedSearchType != null && selectedSearchType.searchType == 'By Customer'?`Customer : ${selectedCustomer!=null?selectedCustomer.customer_name:''}`:''}
        ...
        <TableBody>
            {
              saleProductDetail.map((ele,index)=>(
                ...
                <TableCell align="right">{(ele.sale_qty).toFixed(2)}</TableCell>
                 ...
                 <Link to={{pathname:`/sales/quotation-invoice/${ele.sale_id}`}}> <ReceiptIcon style={{cursor:'pointer',color:'black',marginLeft:'2px'}} > </ReceiptIcon></Link>
              

                 {
    authInfo.role !='user'?(
    <>
         <Link to={{pathname:`/sales/quotation-update/${ele.sale_id}`}}>  <EditIcon style={{cursor:'pointer',color:'green',marginLeft:'2px'}} > </EditIcon></Link>
...

{
  (selectedRecordType !=null && selectedSearchType != null &&  selectedSearchType.searchType !=  'By Product' && selectedSearchType.searchType !=  'By Category' && searchResult.length>0 &&  selectedRecordType.recordType=='Without Details')?(
    ...
    authInfo.role !='user'?(
    <>
                <Link to={{pathname:`/sales/quotation-update/${ele.sale_id}`}}> 
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '2px'}}>Quotation Record</h4>
