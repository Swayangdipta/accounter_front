...
const PurchaseEntry = ({location,currentRoute,currentRouteSet,authInfo})=>{
    ...
    const purchaseToCart = (()=>{
          ...
          purchaseCartSet([...purchaseCart,material]);
    ...
    //   })
      ...
      if(pathSpliter(location.pathname,3) != undefined){
        formActionSet('update')
         axios.post(`${API_URL}/api/get-material-purchase-with-details`,{reqPayload:{purchaseId:parseFloat(pathSpliter(location.pathname,3))}},{headers:{'auth-token':authInfo.token}}).then(res=>{
            let purchase = res.data.message[0];
            let purchaseDetails = res.data.message[0].details;

            selectedEmployeeSet({
              employee_id: purchase.pur_by,
              employee_name: purchase.employee_name
            });

            selectedSupplierSet({
              supplier_id: purchase.supplier_id,
              supplier_name: purchase.supplier_type =='general'?'General Supplier':purchase.supplier_name,
              display_text: purchase.supplier_type =='general'?'General Supplier':purchase.supplier_name,
              supplier_type: purchase.supplier_type
            })

            setPurchaseData({...purchaseData,invoice_no:purchase.m_purchase_invoice,supplier_name:purchase.supplier_name,display_text:purchase.supplier_name,
              supplier_mobile_no:purchase.supplier_mobile_no,supplier_address:purchase.supplier_address,
              pur_transport_cost:purchase.transport_cost,
              pur_supplier_type : purchase.supplier_type,
              pur_supplier_id : purchase.supplier_id,
              pur_note: purchase.note
              
              })
              handleDateChangeSet(purchase.created_isodt)
              pur_vat_amount_set(purchase.vat)
              pur_vat_percent_set(purchase.vat_percent)
              pur_discount_amount_set(purchase.discount)
              pur_discount_percent_set(purchase.discount_percent)
              purTotalAmoSet(purchase.total)
              purPaidAmoSet(purchase.paid)
              purDueAmoSet(purchase.due)



              selectedSupplierPayMethodSet({pay_method_id:purchase.pay_method,pay_method:purchase.pay_method})
              selectedSupplierPayAccSet({bank_display_name:purchase.bank_display_name,bank_acc_id:purchase.bank_id})

           let purchaseCartGet =  purchaseDetails.map((ele)=>{ 
              let material = {
                material_id:ele.material_id,
                material_name:ele.material_name,
                material_purchase_rate:ele.purchase_rate,
                material_qty:ele.purchase_qty,
                material_total:ele.purchase_total
              }
              return material;
            });
    ...
    let savePurchase = async ()=>{
       ...
       let purchase =  {...purchaseData,pur_vat_amount:pur_vat_amount,pur_vat_percent:pur_vat_percent,
        ...
        saveActionSet(true)
        let url = `/save-material-purchase`;
        if(pathSpliter(location.pathname,3) != undefined){
            url =  `/update-material-purchase`;
        }
    

        await axios.post(`${API_URL}/api${url}`,{purchase,purchaseCart},{headers:{'auth-token':authInfo.token}}).then(res=>{
          if(!res.data.error){
            let smg = 'Material Purchase ';
            if(pathSpliter(location.pathname,3) != undefined){
              smg = 'Material Purchase updated '
            }
            swal({
              ...
              if(confirm){
                history.push(`/production/material-purchase-invoice/${res.data.message.productionId}`) 
                saveActionSet(false)
              }else{
    ...
    const getPurchaseInvoice = async ()=>{ 
          await axios.get(`${API_URL}/api/get-material-purchase-invoice`,{headers:{'auth-token':authInfo.token}}).then(res=>{
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '2px'}}>Material Purchase Entry</h4>
...
<Grid container spacing={3} >
        <Grid item xs={12} sm={8} >
          ...
          <h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '3px'}}>Supplier & Material Information</h4>
          <Grid container spacing={3} >
                <Grid item xs={12} sm={6} className={classes.plusLinkDiv}>
                <Link className={classes.plusLink} to="/production/suppliers-manage" >+</Link>  
                <Autocomplete 
                autoHighlight
                size="small"
              openOnFocus={true}
          ...
          />
              ...
              onChange={(e,obj)=>{
                ...
                setMaterial({...material,material_purchase_rate:obj.material_purchase_rate,material_id:obj.material_id,material_name:obj.material_name}) 
        ...
        <br/>
                <Grid container >
                      ...
                      <TextField style={{marginRight:'5px'}}  
                            ...
                            }}  type="number" label="purchase rate"  size="small" value={material.material_purchase_rate} onChange={materialHandle} name="material_purchase_rate" variant="outlined" className={classes.inputField} />
                      </Grid>
                      <Grid item sm={1} >
                      </Grid>
          ...
          <Grid container>
            ...
            <Grid item xs={12}>
                  ...
                  <h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
    marginBottom: '3px'}}>Material Purchase Cart</h4>
                  <TableContainer >
    ...
    <TableContainer >
      ...
      {purchaseCart.length!=0?
          <TableRow >
             <TableCell style={{width: '325px'}}><TextareaAutosize onChange={handleTotalInput} name="pur_note" value={purchaseData.pur_note} style={{float:'left',marginTop:'20px',width: '325px'}} aria-label="Material Purchase Note..." rowsMin={3} placeholder="Material purchase Note..." />
             </TableCell>
