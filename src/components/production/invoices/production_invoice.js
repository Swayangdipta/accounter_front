...
import PurchaseInvoice from './load_production_invoice';
...
const PurchaseInvoiceQuick = ({location,currentRouteSet,authInfo})=>{
    ...
    let getPurchaseInvoices = async ()=>{
        await axios.get(`${API_URL}/api/get-production-invoices`,{headers:{'auth-token':authInfo.token}}).then(res=>{
  ...
  <Grid item xs={12} sm={12}>
        ...
        <Autocomplete 
           ...
           size="small"
           getOptionLabel={(option) => option.production_invoice}
           onChange={(e,invoice)=>{
            selectedInvoiceSet(invoice)
              if(invoice != null){
                  purchaseIdSet(invoice.production_id )
              }
           }}
