...
const CashTransactionHistory = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
      ...
      let payload = {
          materialId: selectedMaterial != null? selectedMaterial.material_id:null,
      ...
      await axios.post(`${API_URL}/api/get-material-ledger`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Material Ledger </p>

  <p style={{float:'left',width:'30%',textAlign:'left',margin:0,padding:0,fontSize:'16px'}}>
    {selectedMaterial != null ?`Material Name : ${selectedMaterial!=null?selectedMaterial.material_name:''}`:''}
    </p>
          ...
          <TableRow  key=""> 
              ...
              
              {
                report.map((row,sl)=>(
                  ...
                  <TableCell  align="left">{moment(row.dateTime).format('Y MMM DD')}</TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Material Ledger</h4> 
