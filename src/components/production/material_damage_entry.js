...
const MaterialDamage = ({location,currentRouteSet,authInfo})=>{
   ...
   let getMaterials = async ()=>{
    ...
    let getDamages = async ()=>{
      await axios.post(`${API_URL}/api/get-material-damages`,null,{headers:{'auth-token':authInfo.token}}).then(res=>{
            damagesSet(res.data)
      })
    }
    ...
    let getDamageCode = async ()=>{
        ...
        await axios.get(`${API_URL}/api/get-material-damage-code`,{headers:{'auth-token':authInfo.token}}).then(res=>{
    ...
    const saveFormAction = async ()=>{
        ...
        let payload = {
            ...
            await axios.post(`${API_URL}/api/save-material-damage`,{payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
    ...
    const damageEdit = (row,index)=>{
      console.log(damages[index])
          damageCodeSet(damages[index].damage_code); 
          selectedMaterialSet({material_id:damages[index].material_id,material_name:damages[index].material_name});
    ...
    const damageDelete = async (index)=>{ 
      ...
      await axios.post(`${API_URL}/api/delete-material-damage`,{damageId},{headers:{'auth-token':authInfo.token}}).then(res=>{
        if(!res.data.error){
          getDamages()
          swal({title:res.data.message,icon:'success'})
        }
    ...
    return (
        <div className={classes.root}>
            ...
            <h2 className={classes.pageEntryLabel}>Material Damage Entry</h2>
             
            <Grid container spacing={2}>
            <Grid item xs={12} sm={2} > 
            <TextField  autoComplete='off'  className={classes.fullWidth}  value={damageCode} 
            label="Material Damage  Code" disabled name="product_damage_code" variant="outlined" size="small"  />
            </Grid>
            ...
            <Autocomplete 
              ...
              onChange={(e,material)=>{
                    ...
                    damageRateSet(parseFloat(material.material_purchase_rate).toFixed(2))
...
const useStyles = makeStyles((theme) => ({
  ...
  export default connect(mapStateToPops,{currentRouteSet})(MaterialDamage);