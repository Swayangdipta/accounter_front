...
const TransferRecord = ({location,currentRoute,currentRouteSet,authInfo})=>{
  ...
  let getSearchResult = async ()=>{
    ...
    if(selectedSupplier == null){
        swal({title:'select a supplier',icon:'warning'});return false;
    }
      ...
      await axios.post(`${API_URL}/api/get-supplier-ledger`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        printSet(true)
...
<p style={{width:'100%',textAlign:'center',fontWeight:'bold'}}>Supplier Ledger </p>
        ...
        <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
          <TableHead>
            <TableRow> 
              ...
              <TableCell align="right" style={{width:'10%'}}>Received </TableCell>
          ...
          <TableRow  key=""> 
                   ...
                   </TableRow>
              {
                ledger.map((row,sl)=>(
                  ...
                  <TableCell  align="right">{format(parseFloat(row.received).toFixed(2))}</TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Supplier Ledger</h4> 
