...
const SupplierList = ({location,currentRoute,currentRouteSet,authInfo})=>{
 ...
 let getSuppliers = async()=>{
  ...
  let getSearchResult = async ()=>{
      ...
      await axios.post(`${API_URL}/api/get-suppliers`,{...payload},{headers:{'auth-token':authInfo.token}}).then(res=>{
        reportSet(res.data.message)
        printSet(true)
      })
        ...
        <Table className={classes.table} size="small" aria-label="a dense table" className="report-dom">
          <TableHead>
            <TableRow> 
              ...
              <TableCell align="left" style={{width:'15%'}}>Supplier ID </TableCell>
              <TableCell align="left" style={{width:'15%'}}>Supplier Name </TableCell>
              <TableCell align="left" style={{width:'15%'}}>Addess </TableCell>
              <TableCell align="left" style={{width:'15%'}}>Mobile No </TableCell>
          ...
          <TableBody>
              ...
              
              {
                report.map((row,sl)=>(
                  ...
                  <TableCell  align="left">{moment(row.supplier_created_isodt).format(dateTimeFormat)}</TableCell>
...
<h4 style={{textAlign:'left',margin:0,padding:0,marginTop: '-10px',
  marginBottom: '2px'}}>Supplier  List</h4> 
...
export default connect(mapStateToPops,{currentRouteSet})(SupplierList);